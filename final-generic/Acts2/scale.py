...
def runs(num):
	...
	if(c > num):
		...
		while(c > num):
			down(ki[list(ki.keys())[-1]], (list(ki.keys())[-1]))
			portVal = portVal - 1
			c = c -1 

def up(portVals):
	global ki
	s1=client.containers.run('run', ports = {'5000/tcp':portVals}, privileged=False, detach=True, network="acts2_default")
...
def killAll():
	for i in client.containers.list() :
		...
		if(not (k.name == "acts2_acts_1") and not (k.name == "acts2_mongo_1")):
			k.kill()
